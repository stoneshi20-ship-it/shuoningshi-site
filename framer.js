...
(function () {
  ...
  var oGrad = document.getElementById("o-grad");
  var gradToggleRow = document.getElementById("gradtoggle-row");
  ...
  var swatchColors = ["#cccccc", "#cccccc", "#cccccc"];
  ...
  function drawPhoto(dx, dy, dw, dh) {
    ...
    // high-quality resampling (canvas defaults to "low", which looks soft/compressed)
  ...
  function tintedMark(color) {
    ...
    // recolour a downscaled copy (the mark is drawn small; full-res per colour-drag froze the UI)
    var cap = 480, sc = Math.min(1, cap / Math.max(markImg.naturalWidth, markImg.naturalHeight));
    var w = Math.max(1, Math.round(markImg.naturalWidth * sc)), h = Math.max(1, Math.round(markImg.naturalHeight * sc));
    var c = document.createElement("canvas"); c.width = w; c.height = h;
    var cc = c.getContext("2d"); cc.drawImage(markImg, 0, 0, w, h);
    ...
    for (var i = 0; i < d.length; i += 4) {
      ...
      d[i + 3] = Math.min(255, Math.round(255 * t * (d[i + 3] / 255) * 1.3)); // opacity +30%
    }
  ...
  function caption3(x0, x1, bandTop, bandH, W, ink, sub, yShift) {
    ...
    var hasSw = swatchColors.length > 0;
    var swBlock = hasSw ? swU : 0;
    var leftH = swBlock + (titleH ? (hasSw ? gapMid : 0) + titleH : 0);
    var lt = mid - leftH / 2;
    if (hasSw) drawSwatches(x0, lt, swU);
    var titleTop = lt + (hasSw ? swU + gapMid : 0);
    var saved = ctx.letterSpacing;
    try { ctx.letterSpacing = Math.round(W * 0.0008) + "px"; } catch (e) {}
    for (var ti = 0; ti < tlines.length; ti++)
      text(tlines[ti], x0, titleTop + titleSz + ti * (titleSz + tGap), titleSz, sub, "left", "400");
  ...
  function render(rb) {
    ...
    if (type === "polaroid") {
      ...
      drawPhoto(0, 0, gw, gh);
      // gradient overlay is optional — off = clean photo, no colour tint
      var gradOn = !oGrad || oGrad.checked;
      if (gradOn) {
      ...
      if (oLabel.checked) {
        var band2 = Math.round(gh * 0.30);
        var pad = Math.round(gw * 0.04);
        // caption sits in the lower part of the gradient, nudged further down
        var capH = Math.round(band2 * 0.62);
        caption3(pad, gw - pad, gh - capH, capH, gw, "#ffffff", "rgba(255,255,255,0.78)", Math.round(band2 * 0.12));
      }
    }
  ...
  function afterExif(f) {
    ...
    i.onload = function () {
      ...
      // only auto-sample the palette on the first photo; keep the user's swatches on re-select
      if (firstLoad) { sampleSwatches(); firstLoad = false; }
  ...
  canvas.addEventListener("pointerdown", function (e) {
    ...
    if (pickIdx >= 0) {
      ...
      try {
        ...
        buildSwatchRow(); // keep the picked colour in its own cell (no reordering)
      } catch (err) {}
  ...
  canvas.addEventListener("wheel", function (e) {
    if (!img) return;
    e.preventDefault();
    scale *= e.deltaY < 0 ? 1.03 : 0.9709;
    scale = Math.max(1, Math.min(10, scale));
    render();
  }, { passive: false });

  swMinus.addEventListener("click", function () {
    if (swatchColors.length > 0) { swatchColors.pop(); buildSwatchRow(); render(); }
  });
  ...
  function syncByType() {
    ...
    if (gradRow) gradRow.style.display = (isGrad && (!oGrad || oGrad.checked)) ? "" : "none";
    if (frameColorRow) frameColorRow.style.display = isGrad ? "none" : "";
  }
  ...
  if (ratioSeg) ratioSeg.addEventListener("click", function (e) {
    var btn = e.target.closest(".seg__btn"); if (!btn) return;
  ...
  if (oGrad) oGrad.addEventListener("change", function () { syncByType(); render(); });
  buildSwatchRow();
  ...
  exportBtn.addEventListener("click", function () {
    ...
    var q = exportFmt === "jpeg" ? (exportSize === "original" ? 0.98 : 0.95) : undefined;
